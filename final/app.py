...
def calculate_body_type():
    try:
        ...
        # Store body type and recommendations in session
...
def get_palette():
    try:
        ...
        Skin color: {skin}
        Lips color: {lips}
        Based on these colors, what is my skin tone color palette in terms of spring, summer, autumn, or winter? which 10 color suits from ['Black' 'Orange' 'Navy Blue' 'Red' 'Beige' 'Yellow' 'Green' 'Pink'
        ...
        'Camel Brown' 'Taupe' 'Bronze' 'Champagne' 'Rose Gold']. i need in one line
        """
...
@app.route('/search')
def search():
    # Retrieve body type and recommendations from session
    body_type = session.get('body_type')
    recommendations = session.get('recommendations')
    palette = session.get('palette')
    return render_template('search.html', body_type=body_type, recommendations=recommendations, palette=palette)
...
def perform_search():
    ...
    return render_template('sresults.html', products=products.to_dict(orient='records'), body_type=body_type, recommendations=recommendations)
...
def get_relevant_products(keyword):
    try:
        ...
        # Convert entire DataFrame to lowercase strings
        df_lower = df.apply(lambda x: x.astype(str).str.lower())
        
        # Split the keyword into individual words
        keywords = keyword.lower().split()
        
        # Create a mask for rows where all keywords are contained in any column
        def contains_all_keywords(row):
            row_str = ' '.join(row)  # Join all column values in the row into a single string
            return all(keyword in row_str for keyword in keywords)
        
        mask = df_lower.apply(lambda row: contains_all_keywords(row), axis=1)
